...
Järgnevas toome sisse \emph{supermatemaatika} põhimõisted ning defineerime
...
Lie algebra mõiste üldistamiseks on vaadelda algebra asemel
\emph{superalgebrat}. Tuletame selleks kõigepealt meelde mõned
baasdefinitsioonid.
...
Kui nullist erineva supervektorruumi elemendi $x \in \V \setminus \{0\}$
korral $x \in \V_{\overline{0}} \cup \V_{\overline{1}}$, siis ütleme, et
...
Sellega on meil olemas piisavad vahendid, et defineerida
...
\end{enumerate}
kus $0 < k < n$, $1 \leq \alpha_1, \alpha_2, \dots, \alpha_n \leq p$,
$1 \leq i_1, i_2, \dots, i_n \leq q$, ja $k < l$ korral
$\alpha_k < \alpha_l$ ning $i_k < i_l$.

Struktuurikonstantide kasutamise, seda eelkõige just ternaarsete Lie
...
\[
    ...
    \sum_{i=1}^{n} |\phi_i|.
\]
Seega on valemi \eqref{naide:end-v-kommutaator} paremal poolel kõigi
...
suluga \eqref{naide:end-v-kommutaator} supervektorruumil $l$-Lie
...
\begin{markus}
    ...
    $\g = \R^2 \oplus \{0\}$, mis on varustatud kommutaatoriga
    ...
    Kirjutades viimases võrduses kommutaatorid definitsiooni järgi
    lahti saame, et mistahes $x \in \R^2$ ja paarisendomorfismide
    korral peaks kehtima võrdus
    \begin{align*}
        ...
        \phi_3 \left( \phi_2 \left( \psi \left(
            \phi_1 \left( x \right) \right) \right) \right),
    \end{align*}
    sest paarisendomorfismide korral gradueeringud sisuliselt
    arvestama ei pea.

    Lineaaralgebrast on hästi teada, et kui
    $M \in \mathrm{Mat}_{m,n} \R$, siis kujutus
    \[ T \colon \R^n \to \R^m, \quad T(v) = M v,\ v \in \R^n, \]
    on lineaarne.
    ...
    \[
        ...
        \begin{pmatrix*}[r] 4 \\ -4 \end{pmatrix*},
        \quad
        \left( \psi \circ [\phi_1, \phi_2, \phi_3] +
        [\psi, \phi_2, \phi_3] \circ \phi_1 \right)
            \begin{pmatrix} 1 \\ 1\end{pmatrix} =
        \begin{pmatrix*}[r] 8 \\ -8 \end{pmatrix*}.
    \]

    Samas $\begin{pmatrix*}[r] 4 \\ -4 \end{pmatrix*} \neq
    \begin{pmatrix*}[r] 8 \\ -8 \end{pmatrix*}$, ja seega sellisel
    kujul üldistus ei kehti. Enamgi veel, meie kontranäitest tuleb
    välja, et lauset \ref{lause:kompositsioon sulus} ei olegi
...
\begin{lau}\label{lause:n-lie salg to (n-1)-lie salg}
...
\begin{markus}
    ...
    maatriksi jälg, ja blokid $A_{00}, A_{11}$ moodustavad paarisosa
    ning blokid $A_{01}, A_{10}$ moodustavad paaritu
...
\begin{proof}
    ...
    \begin{enumerate}[label=\arabic*)]
        \item Oletame, et vektorite $x_1, \dots, x_{n+1}$ hulgas on vaid
            ...
            Viimaks, kui kõik vektorid $x_1, \dots, x_{n+1}$ on paaritud,
            siis $S(x_1) = \dots = S(x_{n+1}) = 0$ ja seega kehtib
            $\phi_S (x_1, \dots, x_{n+1}) = 0$, mis tähendab, et tegu ei ole
            homogeense elemendiga, ning gradueeringute kooskõla tingimus ei
            oma sel juhul mõtet.
...
Artiklis \cite{Abramov:2014} on toodud teoreem, mis ütleb, kuidas saab $n$-Lie
superalgebrast superjälje abil indutseerida $(n+1)$-Lie superalgebra.
...
\begin{lau}\label{lause:indutseeritud n-lie salg on lahenduv}
    Olgu $(\g, [\cdot, \dots, \cdot])$ $n$-Lie superalgebra ja
...
\begin{lau}\label{lause:indutseeritud n-lia alg tsentjada}
    Olgu $n$-Lie superalgebra $(\g, [\cdot, \dots, \cdot])$
    kommutaatori superjälg $S$ ja vastaku superjäljele $S$
    indutseeritud $(n+1)$-Lie superalgebra $(\g_S, [\cdot, \dots, \cdot]_S)$.
